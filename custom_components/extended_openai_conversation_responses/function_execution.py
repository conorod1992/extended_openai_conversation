...
def _validate_number_constraints(name: str, value: int | float, schema: Mapping[str, Any]) -> None:
...
def _validate_object(name: str, value: Mapping[str, Any], schema: Mapping[str, Any]) -> dict[str, Any]:
...
def _validate_value(name: str, value: Any, schema: Mapping[str, Any]) -> Any:
    ...
    if "object" in expected_types and isinstance(value, Mapping):
        ...
        _validate_length_constraint(name, value, schema, "minItems", "maxItems", "items")
        ...
        if items is not None:
            if not isinstance(items, Mapping):
                raise _schema_error(f"items for `{name or 'input'}` must be an object schema")
            value = [
