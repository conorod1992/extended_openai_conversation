...
export function installFunctionDependencyIntegrity(constructor) {
  if (typeof constructor !== "function") {
    const registry = constructor || globalThis.customElements;
    if (!registry) return false;
    if (registry.get?.(PANEL_TAG)) return installFunctionDependencyIntegrity(registry.get(PANEL_TAG));
    registry.whenDefined?.(PANEL_TAG).then(() => installFunctionDependencyIntegrity(registry.get(PANEL_TAG)));
    return true;
  }
