var e=`
  ...
  `;function t(e){if(!e)return``;let t=String(e.title||`Unnamed assistant`).trim(),n=String(e.entry_title||``).trim();return n&&n!==t?`${t} — ${n}`:t}function n(e,t,n,r=``){if(!e)return;let i=e.querySelector(`.${t}`);if(!n){i?.remove();return}i||(i=document.createElement(`p`),i.className=t,e.append(i)),i.replaceChildren();let a=document.createElement(`strong`);a.textContent=`Assistant: `,i.append(a,document.createTextNode(n)),r&&i.append(document.createElement(`br`),document.createTextNode(`Item: ${r}`))}function r(e,r=``){let i=(e.shadowRoot?.querySelector(`#confirm-dialog`))?.querySelector(`.dialog-body`);n(i,`eoc-confirm-scope`,t(e._selectedAgent?.()),r)}export{t as n,r,e as t};