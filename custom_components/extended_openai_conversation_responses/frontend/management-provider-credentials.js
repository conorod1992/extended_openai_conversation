...
export function diagnosticsAuthenticationRejected(result = {}) {
  if (typeof result?.authentication_rejected === "boolean") {
    return result.authentication_rejected;
  }
...
function resetDiagnosticsAfterCredentialUpdate(panel, result) {
...
function ensureApiKeyDialog(panel, agent) {
  ...
  dialog.querySelector("#eoc-api-key-form")?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      input.value = "";
      dialog.close();
      resetDiagnosticsAfterCredentialUpdate(panel, result);
