...
function renderTemporaryMemories(panel) {
  ...
  return `<section class="content-card" data-temporary-memories data-collection-identity="${panel._e(memoryCollectionIdentity(panel))}">
    ...
    ${embeddedFeatureStatusMarkup(panel, "Short-term memory", {state:"available", label:"Expires automatically", summary:"Stored details remain manageable until they expire, including when short-term memory is off."}, {page:"data-memory", subsection:"memory-settings", label:"Configure"})}
