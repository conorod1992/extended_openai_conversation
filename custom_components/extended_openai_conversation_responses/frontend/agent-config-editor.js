...
const field = (panel, key, label, value, type = "text", description = "", disabled = false) => `<label class="setting" data-field="${key}" data-setting data-search="${panel._e(settingSearch(label, description, key))}"><span>${label}</span><input data-config="${key}" data-type="${type}" type="${type === "number" ? "number" : "text"}" value="${panel._e(value)}" ${disabled ? "disabled" : ""}>${description ? `<small>${description}</small>` : ""}<span class="field-error" data-error="${key}"></span></label>`;
const select = (panel, key, label, value, options, description = "", disabled = false) => `<label class="setting" data-field="${key}" data-setting data-search="${panel._e(settingSearch(label, description, key))}"><span>${label}</span><select data-config="${key}" ${disabled ? "disabled" : ""}>${options.map((item) => option(panel, typeof item === "string" ? item : item.value, value, typeof item === "string" ? null : item.label)).join("")}</select>${description ? `<small>${description}</small>` : ""}<span class="field-error" data-error="${key}"></span></label>`;
const toggle = (panel, key, label, value, description = "", disabled = false) => `<label class="config-toggle setting" data-field="${key}" data-setting data-search="${panel._e(settingSearch(label, description, key))}"><span class="setting-copy"><strong>${label}</strong>${description ? `<small>${description}</small>` : ""}</span><span class="switch-control"><input data-config="${key}" data-type="boolean" type="checkbox" role="switch" ${bool(value)} ${disabled ? "disabled" : ""}><span class="switch-track" aria-hidden="true"></span></span></label>`;
...
export function renderConfiguration(panel) {
  ...
  return `<div class="config-toolbar"><input id="config-search" type="search" value="${panel._e(panel._configSearchQuery || "")}" placeholder="Search settings..." aria-label="Search configuration"><div class="agent-actions"><button type="button" class="secondary" id="duplicate-agent" ${cleanOnly}>Duplicate</button><button type="button" class="secondary" id="export-agent" ${cleanOnly}>Export</button><button type="button" class="secondary" id="import-agent">Import</button></div></div>${panel._configDirty ? `<p class="action-help">Duplicate and Export use the saved configuration. Save or revert this shared draft to enable them.</p>` : ""}
    ...
    <div class="content-card config-surface">
      ...
      ${section(panel,"capabilities","Capabilities","Provider and data features available to the agent.","web search skills knowledge memory",`<div class="config-stack">${toggle(panel,"web_search","Web search",config.web_search,"Use the provider's hosted web search when supported.")}<div class="dependent ${config.web_search ? "" : "is-disabled"}" data-dependent="web_search">${select(panel,"web_search_context","Search context",config.web_search_context,choices("web_search_context"),"",!config.web_search)}</div>${toggle(panel,"knowledge_enabled","Knowledge Library",config.knowledge_enabled)}${select(panel,"memory_mode","Memory mode",config.memory_mode,choices("memory_mode"))}${field(panel,"skills","Skills",(config.skills || []).join(", "),"text","Comma-separated installed skill names.")}</div>`)}
      ${section(panel,"archive","Archive","Retain conversations locally for search and continuity.","retention shared session timeout model search",`<div class="config-stack">${toggle(panel,"archive_enabled","Conversation archive",archive)}<div class="dependent ${archive ? "" : "is-disabled"}" data-dependent="archive_enabled"><div class="form-grid">${select(panel,"archive_retention_days","Retention",config.archive_retention_days,choices("archive_retention_days"),"",!archive)}${field(panel,"archive_session_timeout_minutes","Session timeout (minutes)",config.archive_session_timeout_minutes,"number","",!archive)}</div>${toggle(panel,"archive_model_search_enabled","Allow model archive search",config.archive_model_search_enabled,"",!archive)}${toggle(panel,"shared_archive_enabled","Shared-household archive",config.shared_archive_enabled,"",!archive)}</div></div>`)}
      ${section(panel,"voice","Voice & identity","Control how voice requests map to owners and shared data.","unidentified unmapped default owner shared memory satellite device mappings",`<div class="form-grid">${select(panel,"voice_scope_policy","Unidentified voice policy",config.voice_scope_policy,choices("voice_scope_policy"))}${select(panel,"voice_unmapped_policy","Unmapped fallback",config.voice_unmapped_policy,choices("voice_unmapped_policy"))}${field(panel,"voice_default_user_id","Default voice owner",config.voice_default_user_id || "")}${select(panel,"shared_memory_mode","Shared memory mode",config.shared_memory_mode,choices("shared_memory_mode"))}</div><label class="setting mappings-setting" data-setting data-search="satellite device mappings json identity"><span>Satellite/device mappings (JSON)</span><textarea id="voice-mappings" class="yaml-editor mappings-editor" spellcheck="false">${panel._e(JSON.stringify(config.voice_device_mappings || {}, null, 2))}</textarea><small>Map device or satellite IDs to scope owners.</small><span class="field-error" data-error="voice_device_mappings"></span></label>`)}
      ${section(panel,"speech","Spoken response","Clean spoken text without changing the visual response, archive, or memory context.","tts markdown url regex replacements preview",`<div class="config-stack">${toggle(panel,"speech_processing_enabled","Speech post-processing",speech,"Affects spoken output only.")}<div class="dependent ${speech ? "" : "is-disabled"}" data-dependent="speech_processing_enabled">${toggle(panel,"speech_strip_markdown","Remove Markdown links and formatting",config.speech_strip_markdown,"",!speech)}${toggle(panel,"speech_strip_urls","Remove bare URLs",config.speech_strip_urls,"",!speech)}<div class="setting-group" data-setting data-search="custom regex replacements rules pattern replacement"><div class="subheading"><h3>Custom replacements</h3><p>Rules run in order and affect spoken output only.</p></div><div class="rule-headings" aria-hidden="true"><span>Pattern</span><span>Replacement</span><span>Actions</span></div><div id="regex-rules" class="rule-list">${regexRules.map((rule,index)=>regexRow(panel,rule,index,!speech)).join("")}</div><button type="button" class="secondary add-regex" id="add-regex" ${speech ? "" : "disabled"}>+ Add rule</button></div><div class="preview-grid setting-group" data-setting data-search="spoken text preview sample assistant"><label>Sample assistant text<textarea id="speech-sample" class="short-textarea" placeholder="Paste a response containing links or abbreviations" ${speech ? "" : "disabled"}></textarea></label><label>Preview spoken text<textarea id="speech-output" class="short-textarea" readonly></textarea></label></div><button type="button" class="secondary preview-button" id="preview-speech" ${speech ? "" : "disabled"}>Preview spoken text</button></div></div>`)}
      ${section(panel,"context","Context","Choose when and how long conversations are condensed.","threshold truncate summarize recent clear",`<div class="form-grid">${field(panel,"context_threshold","Context threshold",config.context_threshold,"number")}${select(panel,"context_truncate_strategy","Truncation strategy",config.context_truncate_strategy,choices("context_truncate_strategy"))}</div>`)}
      ${section(panel,"model","Model parameters","Only parameters supported by the selected model are shown.","temperature top p reasoning effort service tier tool id memory retrieve",`<div class="form-grid">${capabilities.supports_temperature ? field(panel,"temperature","Temperature",config.temperature,"number") : ""}${capabilities.supports_top_p ? field(panel,"top_p","Top P",config.top_p,"number") : ""}${capabilities.supports_reasoning_effort ? select(panel,"reasoning_effort","Reasoning effort",config.reasoning_effort || "low",choices("reasoning_effort")) : ""}${capabilities.supports_service_tier ? select(panel,"service_tier","Service tier",config.service_tier || "flex",choices("service_tier")) : ""}${toggle(panel,"shorten_tool_call_id","Shorten tool-call IDs",config.shorten_tool_call_id,"Enable only for providers that require short IDs.")}${field(panel,"memory_auto_retrieve_limit","Automatic memory retrieval limit",config.memory_auto_retrieve_limit,"number")}</div><button type="button" class="secondary compact-button section-reset" id="reset-advanced">Reset model parameters</button>`)}
      ${section(panel,"retention","Usage detail retention","Aggregate totals remain; these settings control detailed request and run records.","usage request run details",`<div class="form-grid">${select(panel,"usage_request_retention_days","Request details",config.usage_request_retention_days,choices("usage_request_retention_days"))}${select(panel,"usage_run_retention_days","Run details",config.usage_run_retention_days,choices("usage_run_retention_days"))}</div>`)}
...
function setDependent(root, key, enabled) {
  ...
  container.querySelectorAll("input:not([readonly]),select,textarea:not([readonly]),button").forEach((control) => control.disabled = !enabled);
}
...
export function renderTools(panel) {
  ...
  return `<section class="content-card tools-surface"><div class="section-heading"><div><h2>Function Tools</h2><p>Browse configured tools and edit each complete definition as backend-validated YAML.</p></div><button type="button" id="add-tool">+ Add Function Tool</button></div><div class="list tool-list">${tools.map((tool,index)=>`<article class="list-card tool-card" data-tool-index="${index}"><div class="card-main"><div class="tool-title"><h3>${panel._e(tool.spec?.name||"Unnamed tool")}</h3><span class="type-badge">${panel._e(tool.function?.type||"Unknown type")}</span></div><p class="description">${panel._e(tool.spec?.description||"No description")}</p></div><div class="actions"><button type="button" class="secondary edit-tool" data-index="${index}">Edit</button><button type="button" class="secondary duplicate-tool" data-index="${index}">Duplicate</button><button type="button" class="danger delete-tool" data-index="${index}">Delete</button></div></article>`).join("")||panel._empty("No function tools configured.")}</div><div class="section-actions tools-actions"><button type="button" class="secondary" id="validate-tools">Validate all</button><button type="button" id="save-tools" ${panel._configDirty?"":"disabled"}>Save tools</button><span id="tool-status" class="validation" aria-live="polite"></span></div></section>`;
}
...
export function configurationDialogs(panel) {
  ...
  <dialog id="tool-dialog" class="editor-dialog tool-dialog" aria-labelledby="tool-dialog-title"><div class="dialog-header"><div><h2 id="tool-dialog-title">Function Tool</h2><p id="tool-dialog-meta" class="dialog-meta">YAML</p></div></div><div class="dialog-body tool-dialog-body"><label class="tool-editor-label"><span class="sr-only">Function Tool YAML</span><textarea id="tool-yaml" class="yaml-editor tool-yaml-editor" spellcheck="false" wrap="off" aria-describedby="tool-error"></textarea></label><div id="tool-error" class="validation" role="status" aria-live="polite"></div></div><div class="dialog-actions"><button type="button" class="secondary" id="tool-cancel">Cancel</button><button type="button" class="secondary" id="tool-validate">Validate</button><button type="button" id="tool-save">Keep in draft</button></div></dialog>`;
}
