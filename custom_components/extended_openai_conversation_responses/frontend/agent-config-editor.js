...
export function renderConfiguration(panel) {
  ...
  return `<div class="config-toolbar"><input id="config-search" type="search" value="${panel._e(panel._configSearchQuery || "")}" placeholder="Search settings..." aria-label="Search configuration"><div class="agent-actions"><button type="button" class="secondary" id="duplicate-agent" ${cleanOnly}>Duplicate</button><button type="button" class="secondary" id="export-agent" ${cleanOnly}>Export configuration</button><button type="button" class="secondary" id="import-agent">Import configuration</button></div></div>${panel._configDirty ? `<p class="action-help">Duplicate and Export configuration use the saved configuration. Save or revert this shared draft to enable them.</p>` : ""}
    ...
    <div class="content-card config-surface">
      ...
      ${section(panel,"conversation","Conversation","Control context across separate Assist requests.","conversation continue resume cross-device timeout",`<div class="form-grid">${select(panel,"conversation_continuity","Conversation continuity",config.conversation_continuity,choices("conversation_continuity"),"Controls whether separate Assist requests can reuse recent conversation context.",false,"conversation_continuity")}<div class="dependent ${continuity ? "" : "is-disabled"}" data-dependent="conversation_continuity">${timeoutControl}</div></div>`)}
      ${section(panel,"prompt","Prompt","System instructions used by this agent.","template instructions preview effective rendered",`<label class="setting prompt-setting" data-setting data-search="prompt system instructions template"><span class="sr-only">System prompt</span><textarea id="prompt-editor" data-config="prompt" class="prompt-editor" spellcheck="false">${panel._e(config.prompt || "")}</textarea></label><div class="editor-meta"><span id="prompt-count">${(config.prompt || "").length.toLocaleString()} characters</span><span class="editor-meta-actions"><button type="button" class="secondary compact-button" id="preview-prompt">Preview effective prompt</button><button type="button" class="secondary compact-button" id="reset-prompt">Reset to default</button></span></div><span class="field-error" data-error="prompt"></span>`)}
      ${section(panel,"capabilities","Capabilities","Provider and data features available to the agent.","web search skills knowledge memory temporary ephemeral short-term eager",`<div class="config-stack">${toggle(panel,"web_search","Web search",config.web_search,"Allows the agent to search the web for current information.")}<div class="dependent ${config.web_search ? "" : "is-disabled"}" data-dependent="web_search">${select(panel,"web_search_context","Search context",config.web_search_context,choices("web_search_context"),"Controls how much information the provider returns from web searches.",!config.web_search,"web_search_context")}</div>${toggle(panel,"knowledge_enabled","Knowledge Library",config.knowledge_enabled,"Allows the agent to search locally maintained reference sources; this is separate from persistent memory and conversation history.",false,"knowledge_library")}${select(panel,"memory_mode","Memory mode",config.memory_mode,choices("memory_mode"),"Controls whether durable facts can be remembered and reused across future conversations.",false,"memory_mode")}${select(panel,"temporary_memory","Temporary memory",config.temporary_memory,choices("temporary_memory"),"Lets the assistant silently retain useful short-lived context until it is no longer relevant.",false,"temporary_memory")}${field(panel,"skills","Skills",(config.skills || []).join(", "),"text","Selects reusable installed instruction sets that the agent can load when needed; enter comma-separated names.")}</div>`)}
...
export function bindConfiguration(panel) {
  ...
  root.querySelector("#preview-prompt")?.addEventListener("click", async () => {
    const dialog=root.querySelector("#prompt-preview-dialog");
    const output=root.querySelector("#prompt-preview-output");
    const status=root.querySelector("#prompt-preview-status");
    const sections=root.querySelector("#prompt-preview-sections");
    const notes=root.querySelector("#prompt-preview-notes");
    const copyButton=root.querySelector("#copy-prompt-preview");
    output.value=""; sections.innerHTML=""; notes.innerHTML=""; status.textContent="Rendering current Home Assistant context..."; status.className="validation"; copyButton.disabled=true; dialog.showModal();
    try {
      const result=await panel._call("configuration","prompt_preview",{config:readConfig(panel)});
      output.value=result.prompt;
      sections.innerHTML=(result.sections||[]).map((section)=>`<span class="prompt-section-badge ${panel._e(section.volatility)}">${panel._e(section.label)}</span>`).join("");
      notes.innerHTML=(result.notes||[]).map((note)=>`<li>${panel._e(note)}</li>`).join("");
      status.textContent=`${result.prompt.length.toLocaleString()} characters - resolved using current Home Assistant state`;
  ...
  root.querySelector("#copy-prompt-preview")?.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(root.querySelector("#prompt-preview-output").value);panel._toast("Effective prompt copied");}catch(err){panel._toast(`Unable to copy prompt: ${err.message||String(err)}`,true);}});
...
export function configurationDialogs(panel) {
  return `<dialog id="prompt-preview-dialog" class="editor-dialog wide prompt-preview-dialog" aria-labelledby="prompt-preview-title"><div class="dialog-header"><div><h2 id="prompt-preview-title">Preview effective prompt</h2><p class="dialog-meta">Fresh-request system/context baseline</p></div></div><div class="dialog-body prompt-preview-body"><div class="notice"><strong>Current context preview</strong><p>Shows the system/context content that would be sent for a new conversation right now. Templates and current Home Assistant context are resolved. User input and conversation history are not included.</p><p>This may contain private entity state, memory data, Knowledge context, and user instructions.</p></div><div id="prompt-preview-sections" class="prompt-section-badges" aria-label="Prompt section order"></div><label class="prompt-preview-label">Assembled system/context<textarea id="prompt-preview-output" class="yaml-editor prompt-preview-output" readonly spellcheck="false"></textarea></label><ul id="prompt-preview-notes" class="prompt-preview-notes"></ul><div id="prompt-preview-status" class="validation" role="status" aria-live="polite"></div></div><div class="dialog-actions"><button type="button" class="secondary" id="prompt-preview-close">Close</button><button type="button" id="copy-prompt-preview" disabled>Copy</button></div></dialog>
