...
export function renderConfiguration(panel) {
  ...
  const jumps = [["general","General"],["conversation","Conversation"],["prompt","Prompt"],["capabilities","Capabilities"],["guest","Guest Mode"],["archive","Archive"],["voice","Voice"],["speech","Speech"],["context","Context"],["model","Model"],["retention","Retention"],["backup","Backup & Restore"]];
  return `<div class="config-toolbar"><input id="config-search" type="search" value="${panel._e(panel._configSearchQuery || "")}" placeholder="Search settings..." aria-label="Search configuration"><div class="agent-actions"><button type="button" class="secondary" id="duplicate-agent" ${cleanOnly}>Duplicate</button><button type="button" class="secondary" id="export-agent" ${cleanOnly}>Export configuration</button><button type="button" class="secondary" id="import-agent">Import configuration</button></div></div>${panel._configDirty ? `<p class="action-help">Duplicate and Export configuration use the saved configuration. Save or revert this shared draft to enable them.</p>` : ""}
    ...
    <div class="content-card config-surface">
      ...
      ${section(panel,"guest","Guest Mode policy","Choose the narrow capabilities available while Guest Mode is active. Everything starts private.","guest visitors entity area label domain shared household knowledge",`<div class="config-stack">${toggle(panel,"guest_mode_enabled","Enable Guest Mode voice/model controls",config.guest_mode_enabled,"Exposes the one-way restriction tool. Turning this off does not end an active or scheduled Guest Mode interval; use the trusted Guest page or Home Assistant service to do that.")}${toggle(panel,"guest_shared_memory_read","Allow shared-household memory reads",config.guest_shared_memory_read,"Personal memory is always unavailable in Guest Mode.")}${toggle(panel,"guest_shared_memory_write","Allow shared-household memory writes",config.guest_shared_memory_write,"Personal memory writes remain unavailable.")}${toggle(panel,"guest_knowledge_enabled","Allow Knowledge Library reads",config.guest_knowledge_enabled,"Uses the normal Knowledge Library without personal memory or archive access.")}<div class="form-grid">${field(panel,"guest_readable_entities","Readable entity IDs",(config.guest_readable_entities || []).join(", "),"text","Comma-separated entity IDs guests may read.")}${field(panel,"guest_controllable_entities","Controllable entity IDs",(config.guest_controllable_entities || []).join(", "),"text","Must also be readable.")}${field(panel,"guest_readable_domains","Readable domains",(config.guest_readable_domains || []).join(", "),"text","Comma-separated domains, such as light and climate.")}${field(panel,"guest_controllable_domains","Controllable domains",(config.guest_controllable_domains || []).join(", "),"text","Must also be readable.")}${field(panel,"guest_readable_areas","Readable area IDs",(config.guest_readable_areas || []).join(", "),"text","Comma-separated Home Assistant area IDs.")}${field(panel,"guest_controllable_areas","Controllable area IDs",(config.guest_controllable_areas || []).join(", "),"text","Must also be readable.")}${field(panel,"guest_readable_labels","Readable label IDs",(config.guest_readable_labels || []).join(", "),"text","Comma-separated Home Assistant label IDs.")}${field(panel,"guest_controllable_labels","Controllable label IDs",(config.guest_controllable_labels || []).join(", "),"text","Must also be readable.")}</div></div>`)}
...
function functionToolCard(panel, tool, allTools) {
  ...
  return `<article class="list-card tool-card ${enabled ? "" : "is-disabled"}" data-tool-index="${index}" data-tool-search="${panel._e(`${tool.spec?.name || ""} ${tool.spec?.description || ""} ${tool.function?.type || ""} ${enabled ? "enabled" : "disabled"} ${tool.guest_allowed ? "guest" : "owner only"}`.toLowerCase())}"><div class="card-main"><div class="tool-title"><h4>${panel._e(tool.spec?.name||"Unnamed tool")}</h4><span class="type-badge">${panel._e(tool.function?.type||"Unknown type")}</span>${tool.guest_allowed ? '<span class="type-badge">Guest</span>' : ""}${enabled ? "" : '<span class="disabled-badge">Disabled</span>'}</div><p class="description">${panel._e(tool.spec?.description||"No description")}</p></div><div class="actions tool-card-actions"><label class="tool-enabled-control"><span>Guest</span><span class="switch-control"><input class="tool-guest-allowed" data-index="${index}" type="checkbox" role="switch" aria-label="Allow ${panel._e(tool.spec?.name||"Function Tool")} in Guest Mode" ${tool.guest_allowed ? "checked" : ""}><span class="switch-track" aria-hidden="true"></span></span></label><label class="tool-enabled-control"><span>Enabled</span><span class="switch-control"><input class="tool-enabled" data-index="${index}" type="checkbox" role="switch" aria-label="Enable ${panel._e(tool.spec?.name||"Function Tool")}" ${enabled ? "checked" : ""}><span class="switch-track" aria-hidden="true"></span></span></label><button type="button" class="secondary edit-tool" data-index="${index}">Edit</button><button type="button" class="secondary duplicate-tool" data-index="${index}">Duplicate</button><button type="button" class="danger delete-tool" data-index="${index}">Delete</button></div></article>`;
}
...
function openFunctionGroup(panel, groupId = null) {
  ...
  root.querySelector("#group-guest-allowed").checked=group?.guest_allowed===true;
...
function saveFunctionGroup(panel) {
  ...
  const guest_allowed=root.querySelector("#group-guest-allowed").checked;
  ...
  panel._draft.function_groups=[...others.map((group)=>({...group,functions:(group.functions||[]).filter((functionName)=>!selected.has(functionName))})),{id,name,description,loading_mode,functions,guest_allowed}];
  panel._setConfigDirty(true);
...
export function bindTools(panel) {
  ...
  root.querySelectorAll(".tool-guest-allowed").forEach((input)=>input.addEventListener("change",()=>{panel._draft.functions[Number(input.dataset.index)].guest_allowed=input.checked;panel._setConfigDirty(true);panel._render();}));
