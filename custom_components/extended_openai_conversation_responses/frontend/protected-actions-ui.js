...
export function renderProtectedActions(panel) {
  ...
  return `<section class="page-intro"><div><h1>Protected Actions</h1><p>Add an extra check before Extended OpenAI performs sensitive or easy-to-trigger actions.</p></div><button type="button" id="protected-add">Add protection rule</button></section>
    ...
    <details class="content-card"><summary>Protection boundary</summary><p>Actions run directly by Extended OpenAI are checked before execution. Existing Home Assistant scripts, automations, and scenes can perform additional actions internally that Extended OpenAI cannot always check one by one. Protect the wrapper itself when it performs sensitive work.</p></details>
