...
export function renderConfiguration(panel) {
  ...
  return `<div class="config-toolbar"><details class="agent-actions-menu"><summary aria-haspopup="menu">Agent actions</summary><div role="menu"><button type="button" class="secondary" role="menuitem" id="duplicate-agent" ${cleanOnly}>Duplicate agent</button>${panel._configSectionFilter.has("backup") ? "" : `<button type="button" class="secondary" role="menuitem" id="import-agent">Import configuration</button><button type="button" class="secondary" role="menuitem" id="export-agent" ${cleanOnly}>Export configuration</button>`}</div></details></div>${panel._configDirty ? `<p class="action-help">Duplicate and Export configuration use the saved configuration. Save or revert your settings changes to enable them.</p>` : ""}
    <div class="content-card config-surface">
      ...
      ${section(panel,"backup","Export, Backup, Import & Restore","Move reusable setup or durable agent data safely between agents and installations.","disaster recovery migration memories knowledge usage private",() => renderBackupTransferPanel(Boolean(panel._configDirty)))}
...
export function restoreDialog(panel) {
  return renderRestoreTransferDialog(panel);
}
