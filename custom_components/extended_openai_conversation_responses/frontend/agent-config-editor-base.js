...
const select = (panel, key, label, value, options, description = "", disabled = false, helpKey = null) => `<div class="setting" data-field="${key}" data-setting data-search="${panel._e(settingSearch(label, description, key, helpKey))}">${labelRow(panel, label, key, helpKey, false, value, disabled)}<select id="config-${key}" data-config="${key}" ${disabled ? "disabled" : ""}>${options.map((item) => option(panel, typeof item === "string" ? item : item.value, value, friendlySettingValue(key, typeof item === "string" ? item : item.value) || (typeof item === "string" ? null : item.label))).join("")}</select>${description ? `<small>${description}</small>` : ""}<span class="field-error" data-error="${key}"></span></div>`;
...
function section(panel, id, title, description, keywords, body) {
  ...
  return `<section id="config-${id}" class="config-section" data-config-section data-search="${panel._e(`${title} ${description} ${keywords}`.toLowerCase())}"><div class="config-section-heading"><p class="eyebrow">${title}</p><p>${description}</p></div>${content}</section>`;
}
...
function renderLocalHandling(panel, config) {
  ...
  return `<div class="config-stack">
    ${toggle(panel,"local_intents_enabled","Try Home Assistant before AI",config.local_intents_enabled,"After Request Rules, simple commands Home Assistant already understands can run locally without an AI request. If Home Assistant cannot handle the request, Extended OpenAI continues as normal.",state.supported === false)}
    ...
    <div class="dependent ${config.local_intents_enabled ? "" : "is-disabled"}" data-dependent="local_intents_enabled">
      ${toggle(panel,"local_intent_delayed_commands_to_ai","Send delayed device commands to AI",config.local_intent_delayed_commands_to_ai,"Requests such as “turn off the lights in 20 minutes” can use your Function Tools instead. Normal timers such as “set a 20 minute timer” can still stay local.",!config.local_intents_enabled)}
...
export function renderConfiguration(panel) {
  ...
  return `<div class="config-toolbar"><details class="agent-actions-menu"><summary aria-haspopup="menu">Agent actions</summary><div role="menu"><button type="button" class="secondary" role="menuitem" id="duplicate-agent" ${cleanOnly}>Duplicate agent</button><button type="button" class="secondary" role="menuitem" id="import-agent">Import configuration</button><button type="button" class="secondary" role="menuitem" id="export-agent" ${cleanOnly}>Export configuration</button></div></details></div>${panel._configDirty ? `<p class="action-help">Duplicate and Export configuration use the saved configuration. Save or revert your settings changes to enable them.</p>` : ""}
    ...
    <div class="content-card config-surface">
      ...
      ${section(panel,"local","Local handling","Let Home Assistant handle simple commands locally after Request Rules, then use AI only when needed.","local home assistant intents exceptions request rules delayed commands ai",() => renderLocalHandling(panel,config))}
...
function functionToolCard(panel, tool, allTools) {
  ...
  if (isHALlmTool(tool)) return renderHAToolCard(panel, tool, index);
...
async function openTool(panel,index=null,initialTool=null) {
  ...
  panel._toolOriginalName=index===null?null:panel._draft.functions[index]?.spec?.name||null;
  const root=panel.shadowRoot;
  ...
  try {
    let response;
    if(initialTool){
      response=await panel._call("tools","serialize",{tool:initialTool});
    } else if(index===null){
    ...
    root.querySelector("#tool-error").textContent="Edit the YAML, then save the function.";
