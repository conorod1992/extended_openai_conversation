...
class ExtendedOpenAIManagementPanel extends HTMLElement {
  ...
  async _loadSection(silent = false) {
    ...
    try {
      if (this._section === "overview") {
        ...
        this._result = await this._call("configuration", "get");
        this._draft = JSON.parse(JSON.stringify(this._result.config));
        this._draftTitle = this._result.title;
        this._configDirty = false;
      } else {
        this._result = null;
      }
  ...
  async _navigate(section) {
    if (this._configDirty && ["configuration", "tools"].includes(this._section)) {
      const discard = await this._confirm("Discard unsaved changes?", "Your agent configuration draft has not been saved.", "Discard");
      if (!discard) return;
      this._configDirty = false;
    }
