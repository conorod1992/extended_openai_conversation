...
class ExtendedOpenAIManagementPanel extends HTMLElement {
  ...
  _guestPolicyView(status, policy, state) {
    ...
    return `<section class="notice ${status.currently_active ? "on" : ""}"><strong>Guest Mode status: ${this._e(this._titleCase(state))}</strong><p>${status.active_from ? `Starts ${this._e(this._formatDate(status.active_from))}` : "No interval configured"}${status.active_until ? ` · Ends ${this._e(this._formatDate(status.active_until))}` : status.active_from ? " · No expiry" : ""}</p></section><section class="metric-grid compact">${this._metric("Guest-visible entities", policy.readable_entity_count ?? "—")}${this._metric("Guest-controllable entities", policy.controllable_entity_count ?? "—")}${this._metric("Guest functions", policy.configured_tool_count ?? "—")}${this._metric("Guest archive retention", "Disabled")}</section>${schedule}${legacyNotice}${editor}<details class="content-card"><summary>Capability safety</summary><p>Guest permissions are enforced by Extended OpenAI. Custom, composite, or script capabilities are unavailable when their side effects cannot be safely limited to guest-approved entities. This is intentional.</p></details><section class="notice"><strong>Voice and model safety</strong><p>The assistant can only activate Guest Mode sooner or extend it. Trusted Home Assistant controls can shorten or end it. Model-visible context is fully rebuilt on the next user turn; execution restrictions tighten immediately, but context already sent to a provider cannot be removed retroactively.</p></section>`;
  }
