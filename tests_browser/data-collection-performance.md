...
The standalone Memory panel is also covered because the category chips, importance/scope filters, advanced metadata and clear controls live there, not on the main management route. It uses the same small keyed helper, with separate persistent and temporary maps and `(scope, memory_id)` keys under the current agent/user identity. Category counts and datalist options are a small independently updated strip. Its paginated subclass captures one agent identity for the complete load and rejects superseded responses. The only Python production change registers the shared helper's static URL for that panel; no websocket API or backend semantics change.
