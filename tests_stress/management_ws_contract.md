...
`management_ws_contract.json` lists reviewed mutation and transfer actions. Each entry names a browser journey, WebSocket action, required top-level payload keys, and (where useful) the minimum number of calls. The browser suite checks the actual messages it sent. The Python guard checks the keys against the registered Home Assistant command schemas. UI tests then refresh the page to check authoritative durable state, including deleted objects staying deleted. The existing two-tab genuine-HA suite covers stale writers; provider-wire suites separately check runtime behavior.
