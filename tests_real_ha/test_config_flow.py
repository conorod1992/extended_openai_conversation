    ...
    *,
    ...
    return MockConfigEntry(
        ...
        subentries_data=[
            _subentry(
                "conversation",
                f"{title} Conversation",
                conversation_data if conversation_data is not None else agent_config_defaults(),
            ),
            _subentry(
                "ai_task_data",
                f"{title} AI Task",
                ai_task_data if ai_task_data is not None else dict(DEFAULT_AI_TASK_OPTIONS),
            ),
        ],
    )
...
async def _settle_entry(hass: HomeAssistant, entry: MockConfigEntry) -> None:
    """Wait for HA-triggered reload work after a subentry mutation."""
    await hass.async_block_till_done()
    assert entry.state is ConfigEntryState.LOADED


async def _unload_entry(hass: HomeAssistant, entry: MockConfigEntry) -> None:
    """Explicitly tear down an acceptance entry and its scheduled callbacks."""
    await hass.async_block_till_done()
...
async def test_conversation_subentry_add_uses_real_subentry_manager(
    ...
    try:
        ...
        await _settle_entry(hass, entry)
...
async def test_conversation_subentry_reconfigure_is_panel_only_and_non_destructive(
    ...
    legacy_data = {**agent_config_defaults(), "functions": "legacy-custom-value"}
    entry = _make_entry(conversation_data=legacy_data)
    await _setup_entry(hass, entry)
    try:
...
@pytest.mark.asyncio
async def test_ai_task_subentry_add_basic_path(hass: HomeAssistant) -> None:
    """Create a basic AI Task subentry through HA's subentry manager."""
    entry = _make_entry()
    await _setup_entry(hass, entry)
    try:
        existing_ids = set(entry.subentries)
        ...
        new_ids = set(entry.subentries) - existing_ids
        assert len(new_ids) == 1
        assert entry.subentries[new_ids.pop()].data[CONF_MAX_TOKENS] == 500
    finally:
        await _unload_entry(hass, entry)


@pytest.mark.asyncio
async def test_ai_task_subentry_add_advanced_path(hass: HomeAssistant) -> None:
    """Create an AI Task through HA's two-step advanced flow."""
    entry = _make_entry()
    await _setup_entry(hass, entry)
    try:
        result = await hass.config_entries.subentries.async_init(
...
@pytest.mark.asyncio
async def test_ai_task_reconfigure_basic_updates_real_subentry(
    hass: HomeAssistant,
) -> None:
    """Reconfigure an existing AI Task through HA's direct-save path."""
    entry = _make_entry()
    await _setup_entry(hass, entry)
    try:
...
@pytest.mark.asyncio
async def test_ai_task_reconfigure_advanced_updates_real_subentry(
    hass: HomeAssistant,
) -> None:
    """Reconfigure an existing AI Task through HA's advanced two-step path."""
    entry = _make_entry(
        ai_task_data={
            **DEFAULT_AI_TASK_OPTIONS,
            CONF_CHAT_MODEL: "gpt-4o",
