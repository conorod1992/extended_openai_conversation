...
async def test_same_title_recreation_gets_fresh_storage_and_runtime_identity(
    ...
    options = _options()
    entry = _make_entry(
        "Same Title Recreation",
        include_ai_task=False,
        conversation_options=options,
    )
    # The title itself is deliberately the identity that will be reused. Keep this
    # independent of the parent-entry title generated by the shared test helper.
    original = next(iter(entry.subentries.values()))
    entry.subentries[original.subentry_id] = ConfigSubentry(
        data=original.data,
        subentry_type=original.subentry_type,
        title=_TITLE,
