...
from custom_components.extended_openai_conversation_responses import (
    agent_maintenance, conversation as agent_module, debug, ha_permissions,
    request_static_cache, voice_identity_runtime,
)
from custom_components.extended_openai_conversation_responses.const import DOMAIN, SERVICE_PROCESS
from tests_real_ha.test_acceptance_lifecycle import _make_entry, _setup_entry


async def test_request_entry_identity_and_public_paths_survive_real_setup_reload(hass, hass_admin_user, monkeypatch):
    ...
    entry = _make_entry('Owned Conversation Entry', include_ai_task=False, local_intents=True)
    ...
    with ha_permissions.bind_active_ha_context(outer):
        for reloaded in (False, True):
            ...
            provider = AsyncMock(side_effect=AssertionError('Local request reached provider'))
            monkeypatch.setattr(agent, '_async_handle_message_with_ha_tools', provider)
            manager = debug.get_debug_manager(hass, entry.entry_id, agent.subentry.subentry_id)
            manager.configure(enabled=True)
            before = manager.status()['count']
            result = await conversation.async_converse(
                hass=hass, text='what time is it', conversation_id=None,
                context=caller, language='en', agent_id=entry.entry_id,
            )
            assert result.response.error_code is None
            assert result.conversation_id
            assert ha_permissions.get_active_ha_context() is outer
            direct = await hass.services.async_call(
                DOMAIN, SERVICE_PROCESS,
                {'agent_id': agent.entity_id, 'text': 'what time is it', 'language': 'en'},
                blocking=True, return_response=True, context=caller,
            )
