...
| `functions` | Seeded Function Group loading, disabling and session-isolation model across 24 tools, 12 groups and 16 conversations, a real SDK-wire group load and template-tool execution loop, native provider-wire service execution, delayed due-time HA reauthorization, and direct execution/error tests for all supported Function types. The direct type tests are not counted as provider-wire executions. |
