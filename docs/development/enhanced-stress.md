...
| `functions` | Seeded Function Group loading, disabling and session-isolation model across 24 tools, 12 groups and 16 conversations, plus one real SDK-wire group load and template-tool execution loop. |
...
| `browser` | One mounted Chromium management panel through 80 seeded route changes with recurring Memory, Knowledge and Rule creations checked against fixture backend state, plus two tabs against the same genuine HA backend proving stale Request Rule saves are rejected. |
