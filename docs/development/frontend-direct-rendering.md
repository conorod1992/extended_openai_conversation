...
## Implemented and deliberately retained

Final labels/options/search aliases and the surviving effect/default badges are emitted by configuration and memory field renderers. Model-data cards include their initial availability and status in markup; only real backend responses update them later. Request Rules emits final card summaries and both testers, restore dialogs emit their assistant scope, and Knowledge emits availability badges/editor controls without parsing its HTML back into templates.

Removed: two render wrappers (decision guidance and continuity label), four scheduled enhancement paths (five `queueMicrotask` call sites, including the nested polish pair), and one observer over the owned configuration subtree. This also removes repeated full-field/default/search-result scans, badge construction followed by pruning, model-card reparenting, and two Knowledge template mutation passes. These are concrete removed operations, not a measured latency claim.

The continuity-label installer and asset registration are dead and deleted. Remaining clarity and polish wrappers still handle dirty-navigation projection and Guide layout; decision guidance retains confirmation scope. Generic configuration guidance, navigation/toolbar helpers, shell copy polish, overview health and lazy history pager remain separate candidates. PR5 should assess these surviving installers and bootstrap ordering without treating their still-active behavior as dead code.

Backend model lookups/actions, lazy feature imports, validation responses, dirty-state/lifecycle ordering and the observer of external Home Assistant config-flow DOM remain asynchronous. No state-safety ownership or backend contract changes are included.

Validation before PR creation: all standalone frontend module tests, TypeScript, 31 Vitest tests and production build passed; Chromium passed 73 tests with 13 genuine-HA tests gated for the CI backend environment. New regressions inspect detached markup synchronously and verify one live request after repeated binding/renders. CI supplies the genuine-HA browser, Python/HA matrix, acceptance, HACS, release and metadata checks.
