...
<ParamField path="cwd" type="template">
  Working directory for command execution. Defaults to `config/extended_openai_conversation_responses/`.
</ParamField>
...
<AccordionGroup>
  <Accordion title="Can bash functions modify system files?">
    Yes, if `restrict_to_workspace: false`. By default, modifications are limited to the workspace directory (`config/extended_openai_conversation_responses/`).
  </Accordion>
