...
async def test_public_paths_use_the_same_explicit_stage_order(
    ...
    for name, helper in [
        ("bind_active_ha_context", caller),
        ("conversation_request_lease", lease),
        ("conversation_debug_trace", capture),
        ("formatted_tool_cache", cache),
        ("voice_identity_scope", voice),
        ("async_reconcile_runtime_configuration", reconcile),
    ]:
        monkeypatch.setattr(conversation, name, helper)
    entry_agent._async_process_with_continuity = core
    result = await (
        entry_agent.async_process_direct(entry_input)
        if direct
        else entry_agent.async_process(entry_input)
    )
...
async def test_failures_restore_every_outer_context_and_release_gate(
    entry_agent, entry_input, monkeypatch, stage, failure
):
    entry_agent.subentry.data = {
        CONF_VOICE_SCOPE_POLICY: VOICE_POLICY_DEFAULT_USER,
        CONF_VOICE_DEFAULT_USER_ID: "voice-owner",
    }
    entry_agent.hass.auth.async_get_user = AsyncMock(
    ...
    variables = [
        ha_permissions._ACTIVE_HA_CONTEXT,
        request_static_cache._FORMATTED_TOOLS,
        scope_module._ACTIVE_VOICE_IDENTITY_USERS,
        debug._ACTIVE_DEBUG_TRACE,
        conversation._PROCESS_METADATA,
    ]
    priors = [
        Context(user_id="outer"),
        {"outer": True},
        frozenset({"outer"}),
        object(),
        {"outer": True},
...
async def test_debug_finishes_inside_gate_after_cache_and_voice_restore(
...
async def test_maintenance_wait_precedes_debug_identity_and_live_reconciliation(
    entry_agent, entry_input, monkeypatch
):
    manager = debug.get_debug_manager(entry_agent.hass, "entry", "agent")
    manager.configure(enabled=True)
    ...
    monkeypatch.setattr(
        conversation, "async_reconcile_runtime_configuration", reconcile
    )
    gate = agent_maintenance.get_agent_maintenance_gate(
        entry_agent.hass, "entry", "agent"
    )
...
async def test_cancelled_maintenance_wait_does_not_prepare_request(
    entry_agent, entry_input, monkeypatch
):
    reconcile = AsyncMock()
    monkeypatch.setattr(
...
async def test_concurrent_public_paths_keep_callers_voice_owners_caches_and_capture_isolated(
    ...
    entry_agent.subentry.data = {
        ...
        CONF_VOICE_DEVICE_MAPPINGS: {"left": "user:alice", "right": "user:bob"},
    }
    entry_agent.hass.auth.async_get_user = AsyncMock(
        return_value=SimpleNamespace(is_active=True)
    )
    ...
    try:
        async with asyncio.timeout(2):
            results = await asyncio.gather(
                entry_agent.async_process(left), entry_agent.async_process_direct(right)
            )
    finally:
...
@pytest.mark.parametrize(
    "stage", ["llm_context", "scope", "resolve", "claimed", "release"]
)
@pytest.mark.parametrize("failure", [RuntimeError, asyncio.CancelledError])
async def test_continuity_stage_restores_policy_and_prompt_context(
    stage, failure, entry_input, monkeypatch
):
    policy = SimpleNamespace(guest_active=False)
    resolution = SimpleNamespace(key="key", claim_token="claim")
    continuity = SimpleNamespace(
        async_resolve=AsyncMock(return_value=resolution), async_release=AsyncMock()
    )
    agent = SimpleNamespace(
        subentry=SimpleNamespace(data={}),
        _continuity=continuity,
        _resolve_live_guest_policy=lambda: policy,
        _async_process_claimed=AsyncMock(return_value="done"),
    )
    ...
    try:
        with pytest.raises(failure):
            await conversation.ExtendedOpenAIAgentEntity._async_process_with_continuity(
                agent, entry_input
            )
...
async def test_invalid_timeout_before_claim_does_not_leak_guest_policy(
    entry_input, monkeypatch
):
    continuity = SimpleNamespace(async_resolve=AsyncMock())
    agent = SimpleNamespace(
        subentry=SimpleNamespace(data={CONF_CONVERSATION_TIMEOUT_MINUTES: "invalid"}),
        _continuity=continuity,
        _resolve_live_guest_policy=lambda: SimpleNamespace(guest_active=False),
    )
    monkeypatch.setattr(
        conversation, "resolve_data_scope", lambda *_: SimpleNamespace(device_id=None)
    )
    prior = object()
