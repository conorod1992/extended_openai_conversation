...
async def test_owned_process_uses_fresh_cache_and_restores_outer_context(
...
async def test_owned_process_restores_cache_when_processing_raises(
    entry_agent, entry_input
):
    async def core(request):
...
def test_skill_availability_context_is_scoped_for_tool_and_group_loading(
    monkeypatch,
) -> None:
    agent_type = conversation.ExtendedOpenAIAgentEntity
