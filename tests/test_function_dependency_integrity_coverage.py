...
async def test_recursive_rename_noop_success_and_save_rollback(monkeypatch) -> None:
    ...
    monkeypatch.setattr(integrity.request_rules, "validate_rule", lambda rule: rule)
    service = f"{integrity.DOMAIN}.{integrity.SERVICE_CALL_FUNCTION}"
    original = {
        "id": "one",
        "name": "Rule",
    ...
    assert (
        await integrity.async_rename_function_reference_recursive(manager, "old", "old")
        == 0
    )
    assert (
    ...
    with pytest.raises(RuntimeError, match="save failed"):
        await integrity.async_rename_function_reference_recursive(manager, "old", "new")
    assert manager._rules == [original]


@pytest.mark.asyncio
async def test_request_rule_function_validation_rejects_invalid_dependencies(
    monkeypatch,
) -> None:
    service = f"{integrity.DOMAIN}.{integrity.SERVICE_CALL_FUNCTION}"
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        integrity, "function_tool_enabled", lambda tool: tool["enabled"]
    )
