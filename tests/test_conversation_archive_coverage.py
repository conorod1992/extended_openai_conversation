...
class FakeArchiveStorage:
    """In-memory archive storage with observable writes."""
...
async def test_initialize_salvages_valid_state_around_malformed_records() -> None:
    ...
    result = await archive.async_get("user:alice", valid.session_id)
...
async def test_initialize_completes_valid_pending_transaction_before_loading() -> None:
    storage = FakeArchiveStorage()
    session = _session()
    turn = _turn()
    payload = {"turns": [asdict(turn)]}
    storage.metadata = {
...
async def test_begin_session_expires_old_active_session_and_clamps_inactivity() -> None:
...
async def test_record_turn_validates_and_bounds_text_and_get_pagination() -> None:
    archive = await _archive()
    ...
    result = await archive.async_get(
        "user:alice", session.session_id, start_turn=-4, limit=1
    )
...
async def test_list_sessions_clamps_bounds_orders_and_reports_more() -> None:
    archive = await _archive()
    scope = user_scope("alice", source="authenticated_user")
    first = await archive.async_begin_session(
        "first",
...
async def test_bulk_delete_validation_is_atomic_and_date_range_noop() -> None:
    archive = await _archive()
    scope = user_scope("alice", source="authenticated_user")
    ...
    with pytest.raises(ValueError, match="session_ids must contain"):
        await archive.async_delete_selected(
            "user:alice", [str(index) for index in range(MAX_SEARCH_LIMIT + 1)], confirm=True
        )
    ...
    for start, end in (
        ...
        with pytest.raises(ValueError, match="valid start_date"):
            await archive.async_delete_date_range(
                "user:alice", start, end, confirm=True
            )
...
async def test_delete_selected_deduplicates_ids_and_clears_active_mapping() -> None:
    archive = await _archive()
    session = await archive.async_begin_session(
        "browser",
        user_scope("alice", source="authenticated_user"),
...
async def test_backup_data_omits_unretained_runtime_sessions() -> None:
    ...
    # Convert Bob to an unretained runtime session without persisting transcript content.
    bob = archive.active_session("unretained")
...
def test_validate_backup_rejects_invalid_session_metadata(mutator) -> None:
    ...
    data = {"sessions": [mutator(asdict(session))], "turns": []}

    with pytest.raises(ValueError, match="session metadata is invalid|fields have invalid types"):
        ConversationArchive.validate_backup_data(data, "target-agent")
...
def test_validate_backup_rejects_invalid_turn_metadata(turn_data) -> None:
    session = _session()
    data = {"sessions": [asdict(session)], "turns": [turn_data]}

    with pytest.raises(ValueError, match="turn must be an object|turn is invalid|turn metadata is invalid"):
        ConversationArchive.validate_backup_data(data, "target-agent")
...
async def test_replace_backup_rebuilds_partitions_and_clears_active_state() -> None:
    ...
    assert (await archive.async_get("user:alice", "replacement"))["session"]["turn_count"] == 2


async def test_uninitialized_public_operations_fail_fast() -> None:
...
async def test_async_get_archive_reuses_manager(monkeypatch: pytest.MonkeyPatch) -> None:
    storages = []

    def fake_storage(hass, entry_id: str, subentry_id: str):
        storage = FakeArchiveStorage()
        storages.append((hass, entry_id, subentry_id, storage))
        return storage
    ...
    assert [(item[1], item[2]) for item in storages] == [
        ("entry-1", "agent-1"),
        ("entry-1", "agent-2"),
    ]


def test_home_assistant_storage_constructs_partition_store_and_delegates(monkeypatch) -> None:
    created = []
    ...
    assert created[1][3]["atomic_writes"] is True
...
def test_archive_tools_are_bounded_and_map_to_expected_operations() -> None:
    ...
    assert tools[5]["spec"]["parameters"]["properties"]["session_ids"]["maxItems"] == 50
...
def test_search_snapshot_filters_dates_ranks_phrase_and_paginates() -> None:
    ...
    result = _search_archive_snapshot(
        ((session, turns),), "blue running shoes", "2026-09-01", None, 1, 0
    )
    assert [item["turn_id"] for item in result["results"]] == ["phrase"]
    assert result["has_more"] is False

    token_result = _search_archive_snapshot(
        ((session, turns),), "shoes", None, "2026-09-02", 1, 0
    )
    assert token_result["results"]
    assert token_result["has_more"] is True
    second_page = _search_archive_snapshot(
        ((session, turns),), "shoes", None, "2026-09-02", 1, 1
    )
    assert second_page["results"]
    assert second_page["results"][0]["turn_id"] != token_result["results"][0]["turn_id"]
